...
class MakeModelCommand extends \Webman\Console\Commands\MakeModelCommand
{
    ...
    protected function createModel($class, $namespace, $file, $connection = null)
    {
        ...
        try {
            ...
            $table_plura = str_replace('_model', '', $table_plura);
            $con = Db::connection($connection);
            if ($con->select("show tables like '{$prefix}{$table}'")) {
                $table_val = "'$table'";
                $table = "{$prefix}{$table}";
            } else {
